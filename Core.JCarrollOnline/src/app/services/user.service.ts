...
export class UserService {
    ...
    userExists(userName: string) {
        const url = this.config.apiUrl + '/account/userExists?' + userName;
        return this.http.get(this.config.apiUrl + '/account/userExists?' + userName, this.jwt()).map((response: Response) => response.json());
    }
